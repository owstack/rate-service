...
class Markets extends EventEmitter {
    ...
    async start() {
        ...
        Object.keys(exchanges).forEach((exchangeName) => {
            exchanges[exchangeName].forEach((exPair) => {
                this[exPair.pair] = this[exPair.pair] || {};
                this[exPair.pair][exchangeName] = new Market[exchangeName](exPair);
                promises.push(this[exPair.pair][exchangeName].start());
            });
        });
    ...
    async stop() {
        ...
        Object.keys(exchanges).forEach((exchangeName) => {
            Object.keys(exchanges[exchangeName]).forEach((pairName) => {
                promises.push(this[pairName][exchangeName].stop());
            });
        });
        return Promise.all(promises);
    }
    ...
    getMarketSellPrice(exchanges, pair, requestedQty) {
        ...
        for(var i = 0; i < orderbook.bids.length; i++) {
            ...
            if (totalQty < requestedQty) {
                const neededQty = requestedQty - totalQty;
                console.log(`Need ${neededQty}. OB has ${thisQty} @ ${thisTotalPrice}`);
                if (thisQty < neededQty) {
                    // take it all
                    totalPrice += thisTotalPrice;
                    totalQty += thisQty;
                    console.log('took all');
                } else {
                    // take only needed qty
                    totalQty += neededQty;
                    totalPrice += neededQty * thisPrice;
                    console.log('took some');
                }
            } else {
                break;
            }
        }
        console.log(`Quoted sell price ${requestedQty} @ ${(totalPrice / requestedQty).toFixed(baseCurrency.precision)} = ${totalPrice.toFixed(baseCurrency.precision)} for  on ${exchanges} ${pair}`);
        return totalPrice.toFixed(baseCurrency.precision);
    }

    getOrderBook(exchanges, pair) {
        if(!Array.isArray(exchanges)) {
            return this[pair][exchanges].orderbook;
        }

        if (exchanges.length === 1) {
            return this[pair][exchanges[0]].orderbook;
        }
        ...
        Object.keys(this[pair]).forEach((exchangeName) => {
            if(exchanges.includes(exchangeName)) {
                virtualOrderbook.asks = virtualOrderbook.asks.concat(this[pair][exchangeName].orderbook.asks);
                virtualOrderbook.bids = virtualOrderbook.bids.concat(this[pair][exchangeName].orderbook.bids);
            }
        });
    ...
    getConversionRate(exchanges, pair, requestedValue) {
        ...
        if (requestedValue > maxValue) {
            throw new Error(`Cannot quote conversion rate for value: ${requestedValue} on ${exchange} ${pair}, order book too shallow!`);
        }
