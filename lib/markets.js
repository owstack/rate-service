...
class Markets extends EventEmitter {
    ...
    getConversionRate(exchanges, pair, requestedValue) {
        ...
        if (requestedValue > maxValue) {
            throw new Error(`Cannot quote conversion rate for value: ${requestedValue} on ${exchanges} ${pair}, order book too shallow! depth: ${maxValue}`);
        }
        ...
        console.log(`[debug] Quoted conversion rate for ${requestedValue} ${requestCurrencyCodes[1]} @ ${(totalQty).toFixed(baseCurrency.decimal_digits)} ${requestCurrencyCodes[0]} on ${exchanges} ${pair}`);
        return totalQty.toFixed(baseCurrency.decimal_digits);
    }
}
