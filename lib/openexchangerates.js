...
class OpenExchangeRates extends EventEmitter {
    constructor() {
        ...
        this.refreshRate = (Math.round(Number(config.get('oer_refresh_minutes'))) || 60) * 60 * 1000;
    }
    ...
    convertFromUSD(usdPrice, targetCurrency) {
        const quotes = [];
        ...
        if (this.appId) {
            Object.keys(this.currencies).forEach((code) => {
                if (code !== this.baseCurrency && code !== targetCurrency && this.exchangeRates[code]) {
                    ...
                    currencyQuote.unitRate = math.number(math.bignumber(this.exchangeRates[code])).toFixed(this.currencies[code].decimal_digits);
    ...
    convertToUSD(amount, fromCurrency) {
        ...
        console.log(`[debug] Converting ${  amount  } ${  fromCurrency  } to USD with ${this.exchangeRates[fromCurrency]}`);
        return math.number(math.divide(amount, math.bignumber(this.exchangeRates[fromCurrency]))).toFixed(this.currencies[fromCurrency].decimal_digits);
    }
}
